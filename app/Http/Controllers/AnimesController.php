...
//            dump($request->all());
        ...
        try {
            ...
            for ($i = 0; $i < count($request->post("url")); $i++) {
                ...
                $anime->link_falha = $request->post('link_falha')[$i];
    ...
    public function telaNotificacaoApp(){
        return view("screens.animes.notificacao");
    }

    public function enviarNotificacaoParaApp(Request $request){
        try{
            $topico = $request->post('topic');
            $title = $request->post('title');
            $mensagem = $request->post('body');
            $imagem = $request->post('image');
            $resultado = FirebaseUtil::enviarNotificacao($topico,$title,$mensagem,$imagem);
            alert()->success('pronto','log: '.$resultado);
        }catch (\Throwable $ex){
            \Log::error($ex);
            alert()->error('Houve um erro!',$ex->getMessage());
        }

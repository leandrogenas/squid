<?php $__env->startSection('content'); ?>
    <?php $__env->startComponent("layouts.componentes.box",["title"=>"Publicar Séries","icon"=> "fas fa-video"]); ?>
        ...
        <div class="form-group">
            ...
            <div class="row">
                <?php $__currentLoopData = \App\Enums\Sites::get_serie_sites_active(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $site): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="col-lg-1">
                        <p><?php echo e($site); ?></p>
                        <span class="dot" id="<?php echo e($site); ?>"></span>
                    </div>
...
<?php $__env->startSection("js"); ?>
    <script>
            ...
            if (valor.length > 0) {
                if (valor !== link_inicial) {
                    ...
                    ({
                        type: "POST",
                        dataType: 'json',
                        url: "<?php echo e(route('filmes.pesquisa.serie')); ?>",
                        ...
                        $("#movie_name").val(data.serie);
                        $("#imdb_pesquisa_manual").attr("href","https://www.imdb.com/find?q="+encodeURIComponent(data.serie));
        ...
        function adicionar() {
            ...
            total_tr++;
            $("table > tbody").append(" <tr id='" + total_tr + "'> <td><input type='hidden' name='link_site[]' value='" + link + "'><input type='hidden' name='is_cinema[]' value='false'> <input type='hidden' name='id_themovies[]' value='" + link_the_movie + "'> <input type='hidden' name='movie_name[]' value='" + movie_name+ "'> <input type='hidden' name='id_imdbs[]' value='" + link_imdb + "'>" + link + "</td><td><button type='button' onclick=\"remover('tr#" + total_tr + "')\" class='btn btn-danger'><i class='far fa-times-circle'> Remover</i></button></td></tr>");
            limpar_campos();
        }
        ...
        function enviar_dados() {
            $.post('<?php echo e(route("filmes.store")); ?>', $("#form-publica").serialize(), function (msg) {
                var erro = false;
                if(msg.msg === "sucesso"){
...
<?php echo $__env->make('adminlte::page', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\SyncWeb\resources\views/screens/series/create-series.blade.php ENDPATH**/ ?>